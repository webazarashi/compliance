...
function init_quiz() {
    ...
    var elem, i;
    cstate('intro');
    if(isIPhone && window.parent){
        setInterval(function(){
            lightbox.option({'positionFromTop': jQuery(window.parent).scrollTop()})
        },100);
    }
    save_templates();
    load_config();
    ...
    if (!_cfg.settings.remove_powered_by) {
        $("#intro").after('<div style="position:absolute;top:185px;right:10px;font-size:12px;">Powered by <a href="http://quizgenerator.net" target="_blank">QuizGenerator'
    ...
    if (_cfg.settings.hasOwnProperty('question_count_array')) {
        // ignore master mode if sectioned mode is enabled
...
function convert_image_tags(str) {
    ...
    if (is_mp3_audio_supported()) {
        str = str.replace(/\[\[\[(.*?[.]mp3)\]\]\]/g, '<span>'+
            ...
            '</span>');
        if(isIPhone){
            str = str.replace(/\[\[(.*?[.]mp3)\]\]/g, '<audio class="audio_with_controls" src="$1" preload controls style="vertical-align: middle;margin-left:5px;width:160px; margin-left:40px;"/>');
        }else{
            str = str.replace(/\[\[(.*?[.]mp3)\]\]/g, '<audio class="audio_with_controls" src="$1" preload controls style="vertical-align: middle;margin-left:5px;width:210px; margin-left:40px;"/>');
        }
    } else {
    ...
    return str.replace(/\[\[([^\]]*?)\]\]/g, '<a href="$1" data-lightbox="ex"><img src="$1" class="thumnail"  style="margin: 0px;"/></a>');
}
...
function convert_pulldown(str) {
    'use strict';
    return str.replace(/\{([^\}])\}/g, function(i,j){ 
            ...
            if (student_response === false || student_response.length === 0) {
                ...
                for (i = 0; i < cq.answer.length; i++) {
                    ...
                    for (j = 0; j < answerArray.length; j++){
                        if (student_response[i] == answerArray[j]) {
...
//        $('#instant_response_container').css("overflow", "vislble");
        ...
        $('#instant_response_disp_none').hide();
//        $('#instant_response_container').css("overflow", "visible");
...
function get_score() {
    ...
    if(_cfg.settings.mode == 'normal'){
        ...
        suspend_data_body = suspend_data.substring(4);
        for (i = 0; i < suspend_data_body.length; i += 4) {
            if(parseInt(suspend_data_body.substring(i, i + 2), 10)){
                correct_count++;
            }
            question_count++;
        }
...
setInterval(function(){
	...
	$(".result_table2_box tbody").height(height - 180)
	$(".student_response").width(width - 150);
	$(".student_response div").width(width - 150);
}, 100);

function gotoCertificate(){
	window.open('http://quizgenerator.net/certificate/?type='+_cfg.settings.certificate);
}
